...
DISALLOWED_IDS = (
    [int(id) for id in os.getenv("DISALLOWED_IDS").split(",")]
    if os.getenv("DISALLOWED_IDS")
    else []
)
REGEX = re.compile(
    r"(?i)((pls|plz|plis|ple+ase|pwease|Pwe+ase|plez|ple+ez|ple+ase) )?(pa+t|pwat) (me|mwe)+( (pls|plz|plis|ple+ase|Pwease|Pwe+ase|plez|ple+ez|ple+ase))?"
)

intents = discord.Intents.default()
intents.messages = (
    True  # This allows the bot to receive events about messages being sent and edited
)
